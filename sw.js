...
self.addEventListener("install", e => {
  // Precache fresh copies (no-store) so the offline fallback isn't a stale HTTP-cached file.
  e.waitUntil(
...
self.addEventListener("fetch", e => {
  ...
  // {cache:"no-store"} forces a real network hit so the browser's own HTTP cache
  // (GitHub Pages sends max-age=600) can't hand back a stale copy.
  e.respondWith(
